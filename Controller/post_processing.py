...
def plot_trajectory(xs_norm, ys_norm):
    ...
    plt.plot(
        xs_norm, ys_norm, marker="o", color="blue", linewidth=1, label="Trajectory"
    )
    plt.xlabel("X")
    plt.ylabel("Y")
...
def calculate_curvature_global_spline(
    smooth_xs, smooth_ys, threshold=None, return_s=False, eps=1e-9
):
    ...
    mask = denom_u > eps
    curvature_u[mask] = (
        dx_ds_u[mask] * ddy_ds2_u[mask] - dy_ds_u[mask] * ddx_ds2_u[mask]
    ) / denom_u[mask]
    ...
    if threshold is not None:
        curvature_filtered = np.where(
            np.abs(curvature_full) > threshold, curvature_full, 0.0
        )
        if return_s:
...
def analyze_turns_robust(
    ...
    curvature, s = calculate_curvature_global_spline(
        smooth_xs, smooth_ys, threshold=None, return_s=True
    )
    ...
    for st, ed in merged:
        ...
        apex_radius = (
            float(np.inf) if abs(apex_curv) < 1e-12 else float(1.0 / abs(apex_curv))
        )
...
def analyze_turns(smooth_xs, smooth_ys, curvature=None, threshold=0.08, min_length=18):
    ...
    if curvature is None:
        filtered, s, curvature_full, raw = analyze_turns_robust(
            smooth_xs, smooth_ys, threshold=threshold, min_length_pts=min_length
        )
        return filtered, s
...
def plot_curvature_visual(curvature, threshold=0.01):
    ...
    plt.axhline(
        y=threshold, color="blue", linestyle="--", linewidth=1, label="Threshold (+)"
    )
    plt.axhline(
        y=-threshold, color="blue", linestyle="--", linewidth=1, label="Threshold (-)"
    )
...
def draw_scaled_circle(
    ax, center, radius, color=None, linestyle="--", alpha=0.5, fill=False, linewidth=1.5
):
...
def add_scaled_radius_and_intersections(
    turns_info, smooth_xs, smooth_ys, C=12.0, scale_min=8.0, scale_max=13.0
):
    ...
    for t in turns_info:
        ...
        idxs = find_circle_track_intersections((apex_x, apex_y), scaled_radius, xs, ys)
...
def merge_overlapping_scaled_circles(
    turns_info,
    smooth_xs,
    smooth_ys,
    s=None,
    ...
    while i < len(indexed):
        ...
        merged_turn["apex_coords"] = (float(xs[apex_idx_i]), float(ys[apex_idx_i]))
        ...
        while j < len(indexed):
            ...
            c2 = (float(xs[apex_idx_j]), float(ys[apex_idx_j]))
            ...
            if (
                r2_outer is None
                and (r2_inner is not None)
                and np.isfinite(r2_inner)
                and r2_inner > 0
            ):
            ...
            if r1_inner is not None and r2_inner is not None:
                try:
                    intersect_inner = circles_intersect(
                        c1, float(r1_inner), c2, float(r2_inner), eps=eps
                    )
                except Exception:
                    intersect_inner = False
            ...
            if r1_outer is not None and r2_outer is not None:
                try:
                    intersect_outer = circles_intersect(
                        c1, float(r1_outer), c2, float(r2_outer), eps=eps
                    )
                except Exception:
                    intersect_outer = False
            ...
            if r1_outer is not None and r2_inner is not None:
                try:
                    intersect_outer_inner_1 = circles_intersect(
                        c1, float(r1_outer), c2, float(r2_inner), eps=eps
                    )
                except Exception:
                    intersect_outer_inner_1 = False
            ...
            if r1_inner is not None and r2_outer is not None:
                try:
                    intersect_outer_inner_2 = circles_intersect(
                        c1, float(r1_inner), c2, float(r2_outer), eps=eps
                    )
                except Exception:
                    intersect_outer_inner_2 = False
            ...
            if (
                intersect_inner
                or intersect_outer
                or intersect_outer_inner_1
                or intersect_outer_inner_2
            ):
                ...
                new_start = min(
                    merged_turn.get("start", apex_idx_1), tj.get("start", apex_idx_2)
                )
                new_end = max(
                    merged_turn.get("end", apex_idx_1), tj.get("end", apex_idx_2)
                )
                ...
                new_apex_radius = (
                    float(np.inf)
                    if abs(new_apex_curv) < 1e-12
                    else 1.0 / abs(new_apex_curv)
                )
    ...
    result = add_scaled_radius_and_intersections(
        result, smooth_xs, smooth_ys, C=C, scale_min=scale_min, scale_max=scale_max
    )

    return result


def plot_turns_on_trajectory(
    smooth_xs, smooth_ys, turns_info, show_apex=True, figsize=(8, 8)
):
    plt.figure(figsize=figsize)
    plt.plot(
        smooth_xs, smooth_ys, color="gray", linewidth=1, label="Smoothed trajectory"
    )
    ...
    for i, info in enumerate(turns_info):
        ...
        if show_apex:
            apex_x = (
                float(info["apex_coords"][0])
                if "apex_coords" in info
                else float(smooth_xs[info["apex_idx"]])
            )
            apex_y = (
                float(info["apex_coords"][1])
                if "apex_coords" in info
                else float(smooth_ys[info["apex_idx"]])
            )
            ...
            if scaled_r is not None and np.isfinite(scaled_r):
                ...
                if xidx is not None and 0 <= xidx < len(smooth_xs):
                    ax.plot(
                        ...
                        markersize=8,
                        color=color,
                        label=f"Exit Turn {i+1}",
                    )

    plt.xlabel("X")
    plt.ylabel("Y")
    plt.title(
        "Trajectory with detected turns, apexes and adaptive scaled circle intersections"
    )
    plt.axis("equal")
    plt.grid(True)
...
def compute_turns_vmax_amax(turns_info, mass=1900.0, mu=1.0, g=9.81, downforce=0.0):
    ...
    for info in out:
        ...
        scaled_r = info.get("scaled_radius", None)
        entry_v = _vmax_from_radius(
            a_allowed, scaled_r if scaled_r is not None else apex_r
        )
        exit_v = _vmax_from_radius(
            a_allowed, scaled_r if scaled_r is not None else apex_r
        )
    ...
    for i, t in enumerate(turns_info):
        ...
        if a is None or v is None:
            ...
            if to_kmh and np.isfinite(v):
                print(
                    f"  apex: a_max = {a:.3f} m/s^2, V_max = {v:.3f} m/s ({v*3.6:.1f} km/h)"
                )
        ...
        if ev is None or exv is None or ea is None or exa is None:
            ...
            if to_kmh:
                ...
                print(
                    f"  entry: a_max = {ea:.3f} m/s^2, V_max = {ev:.3f} m/s ({ev_kmh:.1f} km/h)"
                )
                print(
                    f"  exit:  a_max = {exa:.3f} m/s^2, V_max = {exv:.3f} m/s ({exv_kmh:.1f} km/h)"
                )
            else:
...
def generate_new_trajectory_with_hermite(
    xs, ys, turns_info, samples_per_segment=200, blend_points=0
):
    xs = np.asarray(xs)
    ys = np.asarray(ys)
    n = len(xs)
    ...
    for eidx, aidx, xidx, info in turns_sorted:
        ...
        seg_x, seg_y = _build_hermite_segment(
            eidx, xidx, xs, ys, s_arr, num_samples=samples_per_segment
        )
        if seg_x is None:
